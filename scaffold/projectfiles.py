...
def write_setup(project_name, root_dir):
    ...
    setup_path = get_file_path(root_dir, None, "hardhat.config.ts") 
...
def write_inits(project_name, root_dir):
    """Creates all  files necessary for the project skeleton"""
    
    #Create our file paths first...
    init_paths = {
        'test': get_file_path(root_dir, None, "__init__.py"),
        'gitignore': get_file_path(root_dir, None, ".gitignore")
    }
    ...
    gitignore = open(init_paths['gitignore'], 'w')
    gitignore.close()
    print_file(init_paths['gitignore'])
